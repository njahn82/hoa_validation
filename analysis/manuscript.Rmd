...
The initial data source was the Transformative Agreements Public Data from the cOAlition S Journal Checker Tool (JCT), which helps authors identify suitable open access publishing venues. 
...
The resulting JCT data cover agreements active from July 2021 to July 2024.
...
For each database, article metadata was retrieved using all ISSN variants linked to an ISSN-L.
The metadata included DOIs, document types (where available), author roles, institutional affiliations, publication dates and open access information.
...
For Web of Science and Scopus, the earliest publication date was used where available.
In the latter case, the KB determined the earliest known publication date by tracking versions of the raw data.

<!-- literature to check https://direct.mit.edu/qss/article/3/1/99/109079/The-prevalence-and-impact-of-university-->

### Open Access 

Only published versions with Creative Commons licenses available on publisher platforms were considered as hybrid open access. 
hoaddata derived this information from Crossref's license metadata, while Web of Science and Scopus  used data from Unpaywall. 
In additiaon to Crossref, Unpaywall also parses publisher websites directly, because some publishers do not provide machine-readable Creative Commons license information [@Piwowar_2018], although transformative agreement workflows requiring this information during DOI registration [@Geschuhn_2017].
...
```
Author affiliations at the article level were retrieved for both first and, if available, corresponding authors across all databases. 
...
Because neither Web of Science nor Scopus support ROR IDs—the institution identifier used by the JCT—a two-step matching process was implemented to identify articles covered by transformative agreements. 
...
Then, an algorithm selected the most frequent ROR ID and proprietary identifier pairs to handle multiple affiliations and organizational hierarchy differences.
...
As a result of the above-described comprehensive data processing, data-sets were prepared for each database on open access in hybrid journals included in transformative agreements at the country and journal level by year. 
Table \ref{tab:methods_overview_table} presents an general overview of the coverage of hybrid journals in transformative agreements 2019-23 per dataset. 
It shows that the majority of hybrid journals published at least one original article or review (core) in the five-years period.
These journals constituted the basis for the subsequent aggregation of publication metrics. 
While hoad only covers articles with DOI, the aggreation of Scopus and Web of Scienc ewas carried out isng the databse identifier. 
A subsequent compatision of DOU coverage illustrates that non.core articles in Web of Scienc e lack DOIs. These are in aprticular meeting abstracts, a pecularity of this database.
Open access was aggregated using DOI, as all alos Unpaywall only colelctzs opena cces ststaus information for articles with DOI. 
A closer view on core articles with affiliation data reveal a lack of data withr egard to corresponding auhtors in the case of OpenAlex. Only aroud two third of investiaged artilces had at affilation data linked to corresponding authoirs. In case of first authors, the share was higher.
In contrast, web of scienc eand scopus recorded a higher coverage.
Accordingly, only first authro evidence were presented for haoddata in the follwoing
