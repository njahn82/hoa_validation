...
articles_plot <- country_article_plot_df |>
  ...
  ggrepel::geom_text_repel(aes(label = country_code), size = 2) +
...
```{r oa_country_comparision_plot_, fig.asp=1, fig.width = 8.5}
layout <- "EE
AC
BD"
...
most represented are chinese, usa and india in temrs of articles.
however, in terms of oa, other country lead,
these are in particular those with transformative agremments

overall, briscs and oecd, however, notable expection is south africa

en though correlation exisits, a closer lokk at some temproarl trend revela (dahsboard), which countries were affected by inconsitstent oa evidnecie.
heres is wiley, and there is france.
