...
jn_upset_publisher_ind <- 
  ...
  group_by(publisher) |> 
  summarise(n_journals = n_distinct(issn_l), articles = sum(overlap))
```

Figure \ref{fig-upset_coverage_results_publisher} illustrates the coverage of hybrid journals in transformative agreements by publisher portfolio according to the ESAC registry between 2019 and 2023.
Elsevier was best represented in the union set of all investigated databases, with `r jn_upset_publisher_ind |> filter(publisher == "Elsevier") |> pull(n_journals)` hybrid journals with at least one open access article that published `r jn_upset_publisher_ind |> filter(publisher == "Elsevier") |> pull(articles)` articles between the five-year period. Springer Nature followed with `r jn_upset_publisher_ind |> filter(publisher == "Springer Nature") |> pull(n_journals)` hybrid journals publishing `r jn_upset_publisher_ind |> filter(publisher == "Springer Nature") |> pull(articles)` articles, and Wiley with `r jn_upset_publisher_ind |> filter(publisher == "Wiley") |> pull(n_journals)` hybrid journals and `r jn_upset_publisher_ind |> filter(publisher == "Wiley") |> pull(articles)` articles.
The remaining `r jn_upset_publisher |> filter(publisher == "Other") |> distinct(esac_publisher) |> nrow()` publishers recorded  a total of `r jn_upset_publisher |> filter(publisher == "Other") |> distinct(issn_l) |> nrow()` hybrid journals that published  `r jn_upset_publisher_ind |> filter(publisher == "Other") |> pull(articles)` articles.

Notably, the three largest publishers--Elsevier, Springer Nature and Wiley--were well represented in the exclusive intersection of all three data sources examined.
In terms of journal coverage, they accounted for `r jn_upset_publisher_ind |> filter(publisher != "Other") |> pull(n_journals) |> sum()` (`r jn_upset_publisher_ind |> filter(publisher != "Other") |> pull(n_journals) |> sum() / nrow(union_jns)*100`%) of the hybrid journals included in the intersection.
This dominance is even more pronounced in terms of article coverage, with these publishers having a joint indexing coverage of `r jn_upset_publisher_ind |> filter(publisher != "Other") |> pull(articles) |> sum()` articles (`r jn_upset_publisher_ind |> filter(publisher != "Other") |> pull(articles) |> sum() / upset_data |> filter(Crossref == TRUE, WoS == TRUE, Scopus == TRUE) |> pull(overlap) |> sum()*100`%), as measured by shared DOIs. 
...
When considering journals exlcusively covered by Scopus and Web of Science

When considering journals exlcusively covered by hoaddata
