...
Table @tbl-methods_overview_tab provides a general overview of the coverage between 2019 and 2023 per database.
...
Overall, % (n = ) of hybrid journals included in transformative agreements were indexed in all three databases (Fig A).
These were followed by the combination between hoaddata and Scopus, which indexes more journals than the investiaged Web of Science collection SCIE, SSCI and the Arts & Humanities Citation Index AHCI. 
Around % of journals were only indexed by hoaddata. 
Journals 

Note that there were 32 journals were indexed in WoS or Scopus.
Upon isnepction, these were due to diasmbiguity of ISSNs matching (errors ISSN).  


```{r coverage_results, fig.asp=0.75}
...
jn_upset_articles |> group_by(Crossref, WoS, Scopus) |> summarize(across(.cols = where(is.numeric),.fns =sum, na.rm = TRUE)) 
