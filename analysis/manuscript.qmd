...
This section examines the uptake of open access in hybrid journals, focussing on the influence of transformative agreements across hoaddata, Scopus and Web of Science.
The analysis aims to determine whether consistent results can be derived from these three data sources despite their differing coverage and methodologies. 
Following @Jahn_2025, indicators include the number and proportion of open access articles, both overall and those specifically enabled by transformative agreements. 
The analysis compares the development of these indicators over time between 2019 and 2023 across all three data sources.
...
In 2023, hybrid open access uptake reached  `r uptake_by_year_short |> filter(src == "HOAD", earliest_year == 2023) |> pull(oa_prop)*100`%  in hoaddata (n = `r uptake_by_year_short |> filter(src == "HOAD", earliest_year == 2023) |> pull(n_oa_articles)`) and Scopus (n = `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2023) |> pull(n_oa_articles)`), and `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2023) |> pull(oa_prop) * 100`%  in Web of Science (n = `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2023) |> pull(n_oa_articles)`.

The number and proportion of open access articles enabled through transformative agreements increased between 2019 and 2023 (Figures \ref{fig-uptake_overview}B and C).
These trends remained consistent when estimating open access via first author (dashed line) and corresponding author (dotted line) affiliation metadata. 
Using Scopus, it was possible to attribute `r uptake_by_year_short |> filter(src == "Scopus") |> pull(ta_oa_first_author_articles) |> sum()` open access articles to transformative agreements based on first author metadata (increasing from `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2019) |> pull(ta_oa_first_author_articles) |> sum()` to `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2023) |> pull(ta_oa_first_author_articles) |> sum()`) and `r uptake_by_year_short |> filter(src == "Scopus") |> pull(ta_oa_corresponding_author_articles) |> sum()` using corresponding author metadata (from `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2019) |> pull(ta_oa_corresponding_author_articles) |> sum()` to `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2023) |> pull(ta_oa_corresponding_author_articles) |> sum()`).
Web of Science recorded `r uptake_by_year_short |> filter(src == "Web of Science") |> pull(ta_oa_first_author_articles) |> sum()` open access articles via transformative agreements using first author metadata ( growing from `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2019) |> pull(ta_oa_first_author_articles) |> sum()` to `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2023) |> pull(ta_oa_first_author_articles) |> sum()`)  and  `r uptake_by_year_short |> filter(src == "Web of Science") |> pull(ta_oa_corresponding_author_articles) |> sum()` using corresponding author metadata (from `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2019) |> pull(ta_oa_corresponding_author_articles) |> sum()` to `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2023) |> pull(ta_oa_corresponding_author_articles) |> sum()`).
The development over the years for hoadata is shown only for first authors due to unavailable corresponding author data. 
In total, hoaddata linked `r uptake_by_year_short |> filter(src == "HOAD") |> pull(ta_oa_first_author_articles) |> sum()` open access articles to transformative agreements, making it the most comprehensive data source for first-author analysis. 
However, hoaddata showed slower growth compared to the other two data sources, as illustrated by its fewer articles than Scopus in 2023. 
Furthermore, Scopus and Web of Science recorded slighlty fewer articles for first author articles than those reported for corresponding authors.

When considering the percentage of hybrid open access through transformative agreements relative to total open access, all data sources consistently indicate that between 2021 and 2022, the majority of open access was enabled by transformative agreements.
This trend can be obsverved for both first and corresponding author affiliations.
For first authors, the share of open access ranged between `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2023) |> pull(ta_oa_prop) * 100` % (Web of Science),  `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2023) |> pull(ta_oa_prop) * 100`% (Scopus) and  `r uptake_by_year_short |> filter(src == "HOAD", earliest_year == 2023) |> pull(ta_oa_prop) * 100`% (hoaddata).
For corresponding authors, the share was slighly larger, with Web of Science recording `r uptake_by_year_short |> filter(src == "Web of Science", earliest_year == 2023) |> pull(ta_oa_cor_prop) * 100` % and Scopus  `r uptake_by_year_short |> filter(src == "Scopus", earliest_year == 2023) |> pull(ta_oa_cor_prop) * 100`%)
Nevertheless, substantial amounts of hybrid open access were still facilitated by means other than transformative agreements, likely through APCs invoiced to researchers and paid from research funds.

Despite these similarities, the results also reveal notable differences. 
Specifically, the Web of Science recorded substantially higher open access uptake in the early years compared to Scopus and hoaddata, before converging in 2023. This suggests that delayed open access, i.e., open access after an embargo period, was labeled as hybrid open access in the Web of Science.

### Open access by publishers

When considering open access trends by publisher, it becomes evident that in particular due to Elsevier, the largest publsiher in our sample. 
Following @Jahn_2025, Figure ...
